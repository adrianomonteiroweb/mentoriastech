...
describe("step config helpers", () => {
  it("all mentoring types should have 4 steps", () => {
    expect(getTotalSteps("free")).toBe(4);
    expect(getTotalSteps("paid")).toBe(4);
    expect(getTotalSteps("private")).toBe(4);
  });

  it("free flow should return free step labels", () => {
    expect(getStepLabels("free")).toEqual(STEP_LABELS);
    expect(getStepLabels("private")).toEqual(STEP_LABELS);
  });

  it("paid flow should return paid step labels", () => {
    const paidLabels = getStepLabels("paid");
    expect(paidLabels).toHaveLength(4);
    expect(paidLabels[0]).toBe("Tema");
    expect(paidLabels[3]).toBe("Pagamento");
  });

  it("STEP_LABELS (free) should end with Confirmação", () => {
