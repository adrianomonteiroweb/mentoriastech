...
const mockTopics: TopicItem[] = [
  { id: "t1", name: "Carreira em programação", category: "free", description: "Orientação de carreira" },
  { id: "t2", name: "Desenvolvimento Web", category: "free", description: null },
  { id: "t3", name: "Automações RPA", category: "free", description: "Automação com Python" },
]
...
describe("TopicStep", () => {
  ...
  it("should show loading state", () => {
    ...
    expect(screen.queryByText("Carreira em programação")).not.toBeInTheDocument()
  })

  it("should show empty state when no topics", () => {
    render(<TopicStep {...defaultProps} topics={[]} />)

    expect(screen.getByText("Nenhum tema disponível no momento.")).toBeInTheDocument()
  })
  ...
  it("should call onSelect when clicking a topic", async () => {
    ...
    expect(onSelect).toHaveBeenCalledWith("t1", "Carreira em programação")
  })
